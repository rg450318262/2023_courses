...
social_remove_empty <- 
  social |> 
  filter(is.na(Q30) | Q30 == -99 | is.na(BG_Q_leader))

remove_ept <- social_remove_empty$index

social <- 
  social |>
  filter(!index %in% remove_ept)
```
...
```{r}
#sofa_noleader |> apply(2, function(x)tabyl(x))
```
...
sofa_ldr20_fa3_ft <- fa(sofa_ldr20_ft[-1], nfactors = 3, rotate = "varimax", fm = "ml", scores = "regression")
fa.diagram(sofa_ldr20_fa3_ft, cut = 0, digits =2, main = "Factor Analysis, Varimax rotation")
loading_ldr20_fa3_ft <- sofa_ldr20_fa3_ft$loadings[1:13,]
...
```{r}
ldr20_fa3_cv_ft <- sofa_ldr20_fa3_ft$Vaccounted[3,3] #cv is for cumulative variance
ldr20_fa3_cv <- sofa_ldr20_fa3$Vaccounted[3,3]
ldr20_fa4_cv <- sofa_ldr20_fa4$Vaccounted[3,4]
ldr20_fa5_cv <- sofa_ldr20_fa5$Vaccounted[3,5]
